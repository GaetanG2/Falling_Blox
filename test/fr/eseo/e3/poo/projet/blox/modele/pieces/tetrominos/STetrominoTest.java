...
class STetrominoTest {

	// test du constructeur de Stetromino
			@Test
			public void testConstructeur() {
				Coordonnees cord = new Coordonnees(4, 5);
				STetromino tetro = new STetromino(cord, Couleur.BLEU);
			...
			public void testSetter() {
				...
				STetromino tetro = new STetromino(new Coordonnees(0, 0), Couleur.BLEU);
			...
			public void testSetPosi() {
				...
				assertEquals(new Element(7, 5, Couleur.BLEU), elemt[1]);
				assertEquals(new Element(6, 5, Couleur.BLEU), elemt[2]);
				assertEquals(new Element(5, 6, Couleur.BLEU), elemt[3]);

			}
			//Test methode tourner
			@Test
		    public void testTourner() throws BloxException {
				STetromino tetro = new STetromino(new Coordonnees(3, 3), Couleur.ROUGE);
				STetromino tetro1 = new STetromino(new Coordonnees(3, 3), Couleur.BLEU);


		        tetro.tourner(true); 
		        Element[] elemt = tetro.getElements();
		        assertEquals(new Element(new Coordonnees(3, 3), Couleur.ROUGE), elemt[0]);
		        assertEquals(new Element(new Coordonnees(4, 4), Couleur.ROUGE), elemt[1]);
		        assertEquals(new Element(new Coordonnees(4, 3), Couleur.ROUGE), elemt[2]);
		        assertEquals(new Element(new Coordonnees(3, 2), Couleur.ROUGE), elemt[3]);
		        
		        tetro1.tourner(false); 
		        Element[] elemt1 = tetro1.getElements();
		        assertEquals(new Element(new Coordonnees(3, 3), Couleur.BLEU), elemt1[0]);
		        assertEquals(new Element(new Coordonnees(2, 2), Couleur.BLEU), elemt1[1]);
		        assertEquals(new Element(new Coordonnees(2, 3), Couleur.BLEU), elemt1[2]);
		        assertEquals(new Element(new Coordonnees(3, 4), Couleur.BLEU), elemt1[3]);
		    }
			@Test
			// test de la representation textuelle
			public void testToString() {
				STetromino tetro = new STetromino(new Coordonnees(3, 5), Couleur.BLEU);

				// assertEquals(null,tetro.getElements());
				assertEquals("STetromino :\n" + "	" + "(3, 5) - BLEU" + "\n" + "	" + "(4, 4) - BLEU" + "\n" + "	"
						+ "(3, 4) - BLEU" + "\n" + "	" + "(2, 5) - BLEU" + "\n", tetro.toString());
			}
